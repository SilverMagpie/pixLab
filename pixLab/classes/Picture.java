...
public class Picture extends SimplePicture 
{
  ...
  public void almostInvert(int size) {
   Pixel[][] pixels = this.getPixels2D();
   for (int i=0;i<pixels.length;i++) {
      ...
      for (int j=0;j<row.length;j++) {
         ...
         int a=i+1;
         int b=j+1;
         if (i%(size*2)<size&&j%(size*2)<size) {
            p.setRed(p.getRed()+i%size+j%size);  
            p.setGreen(p.getGreen()+i%size+j%size);
            p.setBlue(p.getBlue()+i%size+j%size);
         }
         else {
            p.setRed(p.getRed()-i%size-j%size);  
            p.setGreen(p.getGreen()-i%size-j%size);
